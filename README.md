...
> **不会开发或打包？** 打开本仓库 GitHub 页面**右侧的 Releases**，直接下载安装包即可使用。  
> 本工具为**纯离线项目**，识别与数据均在本机完成，**无需担心数据泄露**。

选一个放发票的文件夹，工具会自动扫描里面的 **`PDF` / `JPG` / `JPEG` / `PNG`** 文件（当前仅支持这四种格式），用本地 OCR 把发票信息读出来，存到本地数据库，之后可以搜索、编辑、导出成 Excel。

---

## 能做什么

- **批量扫描**：选择文件夹，自动处理 **`PDF` / `JPG` / `JPEG` / `PNG`** 发票文件
- **纯离线 OCR**：本机识别，不上传外网，支持常见电子发票、铁路电子客票等
